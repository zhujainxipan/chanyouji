...
public class StickyListHeadersListView extends FrameLayout {
    ...
    public interface OnStickyHeaderOffsetChangedListener {
         ...
         * @param header The currently sticky detail_travel_group_header being offset.
         *               This detail_travel_group_header is not guaranteed to have it's measurements set.
         ...
         * @param offset The amount the sticky detail_travel_group_header is offset by towards to top of the screen.
         */
        void onStickyHeaderOffsetChanged(StickyListHeadersListView l, View header, int offset);
    }

    /**
     * Notifies the listener when the sticky detail_travel_group_header has been updated
     */
    public interface OnStickyHeaderChangedListener {
        /**
         * @param l             The view parent
         * @param header        The new sticky detail_travel_group_header view.
         * @param itemPosition  The position of the item within the adapter's data set of
         *                      the item whose detail_travel_group_header is now sticky.
         * @param headerId      The id of the new sticky detail_travel_group_header.
         */
...
//                }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        // The detail_travel_group_header should be drawn right after the lists children are drawn.
        // This is done so that the detail_travel_group_header is above the list items
    ...
    // Reset values tied the detail_travel_group_header. also remove detail_travel_group_header form layout
    ...
    private void updateHeader(int firstVisiblePosition) {
        ...
        // check if there is a new detail_travel_group_header should be sticky
        if (mHeaderPosition == null || mHeaderPosition != firstVisiblePosition) {
            ...
            if (mHeaderId == null || mHeaderId != headerId) {
                ...
                if (mHeader != header) {
                    if (header == null) {
                        throw new NullPointerException("detail_travel_group_header may not be null");
                    }
                    swapHeader(header);
                }
                ...
                // that it will be set on the detail_travel_group_header and
                // not skipped for performance reasons.
                mHeaderOffset = null;
            }
        }

        //自己加入的,用来设置actionbar的位置
        int headerOffset = getActionBarHeight();

        // Calculate new detail_travel_group_header offset
        ...
        for (int i = 0; i < mList.getChildCount(); i++) {
            ...
            if (child.getTop() >= (mClippingToPadding ? mPaddingTop : 0)
                ...
                //headerOffset = Math.max(child.getTop() - headerBottom, 0);
                //headerOffset = 500;
    ...
    // hides the headers in the list under the sticky detail_travel_group_header.
    ...
    private void updateHeaderVisibilities() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            // ensure wrapper view child has a detail_travel_group_header
            WrapperView wrapperViewChild = (WrapperView) child;
            if (!wrapperViewChild.hasHeader()) {
                continue;
            }

            // update detail_travel_group_header views visibility
    ...
    // Wrapper around setting the detail_travel_group_header offset in different ways depending on
    // the API version
    @SuppressLint("NewApi")
    ...
    private void setHeaderOffet(int offset) {
        if (mHeaderOffset == null || mHeaderOffset != offset) {
            ...
            //mHeaderOffset = getActionBarHeight();
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
    ...
    public int getActionBarHeight() {
        ...
        getContext().getTheme().resolveAttribute(android.R.attr.actionBarSize, mTypedValue, true);
        return TypedValue.complexToDimensionPixelSize(mTypedValue.data, getResources().getDisplayMetrics());
    }
    ...
    private class WrapperViewListLifeCycleListener implements LifeCycleListener {
        ...
        public void onDispatchDrawOccurred(Canvas canvas) {
            ...
            // therefor we need to update the detail_travel_group_header here as well.
